	...
	for _, schedule := range d.schedules {
		if schedule.Enabled && (schedule.Type == "" || schedule.Type == "discharge") {
			if d.isTimeToDischarge(schedule.StartTime, schedule.StopTime) {
				// Check run_once logic: if enabled and goal was reached, check if it's a new calendar day
				if schedule.RunOnce {
					if goalTime, reached := d.goalReached[schedule.Name]; reached {
						// Check if goal was reached on a previous calendar day
						goalDay := goalTime.In(d.timezone).Truncate(24 * time.Hour)
						today := now.Truncate(24 * time.Hour)

						if today.After(goalDay) {
							// New day, clear the goal state
							...
							).Info("new day started, cleared goal reached state for run_once schedule")
						} else {
							// Same day, skip this schedule
							d.log.With(
								slog.String("schedule", schedule.Name),
								slog.Time("goal_reached_at", goalTime),
							).Debug("run_once schedule already completed today, skipping")
							continue
						}
					}
				}
...
func (d *Discharge) processControlCommand(cmd ControlCommand) error {
	...
	case CommandResetGoal:
		...
		// Clear goal state
		if _, exists := d.goalReached[cmd.ScheduleName]; exists {
			delete(d.goalReached, cmd.ScheduleName)
			if d.clearGoalReached != nil {
